...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--set-password',
            action='store_true',
            help='If set, the loaded users will have their password set to a random value.'
        )
        parser.add_argument(
            '--has-headers',
            action='store_true',
            default=True,
            help='If set, the CSV file is expected to have headers. If not set, the first row will be treated as data.'
        )

    @signalcommand
    def handle(self, *args, **options):
        endpoint: str = options.get('url', None)
        filename: str = options.get('filename', None)
        has_headers: bool = options.get('has_headers', True)

        if filename is None and endpoint is None:
            raise CommandError(
                'You must provide either a file path or a URL to load the users from.'
            )

        if filename is not None and endpoint is not None:
            raise CommandError(
                'You cannot provide both a file path and a URL to load the users from. Please choose one.'
            )

        if filename.endswith('.csv'):
            raise CommandError('Provide a CSV file name without the .csv extension. The command will automatically append it.')

        base_dir: pathlib.Path = settings.BASE_DIR
        file_path: pathlib.Path = base_dir.joinpath(f'{filename}.csv')
            ...
            for idx, row in enumerate(reader):
                ...
                if not options['dry_run']:
                    ...
                    if options['format_values']:
                        defaults = {
                            k: v.strip().title() if isinstance(v, str) else v
                    ...
                    if qs.exists():
                        ...
                        # Set None usernames to a default value
                        if defaults['username'] is None:
                            defaults['username'] = f'user_{get_random_string(length=8)}'
